...
**/
final class UIStepper extends UIComponent implements ui.input.IUIFocusable {
		...
		switch (keyCode) {
			...
			default:
				...
				if (keyCode >= 48 && keyCode <= 57)
					...
					ch = '-';
				if (ch == '-') {
					editBuffer = editBuffer.startsWith('-') ? editBuffer.substr(1) : '-$editBuffer';
					invalidate();
				} else if (ch != null) {
