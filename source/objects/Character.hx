...
class Character extends FlxSprite {
	...
	**/
	...
	/** Whether this character re-fires its sing animation each step while a sustain is held -- the classic
		pre-v2 hold "jitter" (`PlayState.resingHold` drives it). On by default; set the character json's
		`loop_sing_on_hold` to `false` to instead hold the head's sing pose frozen through the sustain. **/
	public var loopSingOnHold:Bool = true;
	...
	public function loadCharacterFile(json:Dynamic) {
		...
		loopSingOnHold = (json.loop_sing_on_hold != false); // on by default; explicit false freezes the pose
	...
	override function update(elapsed:Float) {
		...
		// A character with a dedicated `<anim>-loop` still plays it when the base anim finishes; the hold
		// "jitter" (re-firing the sing each step) is driven by PlayState.resingHold, gated on loopSingOnHold.
	...
	@:allow(editors.mobile.MobileCharacterEditorState)
